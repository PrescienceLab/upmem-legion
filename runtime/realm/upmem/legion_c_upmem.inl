...
//----------------------------------------------------------------------------
template <int DIM, typename COORD_T>
inline COORD_T Legion::PointInRectIterator<DIM, COORD_T>::operator[](unsigned index) const
//----------------------------------------------------------------------------
{
  return itr.p[index];
}
...
class FieldAccessor<LEGION_READ_ONLY, FT, N, T, Realm::AffineAccessor<FT, N, T>, true> {
private:
  static_assert(N > 0, "DIM must be positive");
...
class FieldAccessor<LEGION_READ_ONLY, FT, 1, T, Realm::AffineAccessor<FT, 1, T>, CB> {
public:
  FieldAccessor(void) {}
...
class FieldAccessor<LEGION_READ_ONLY, FT, 1, T, Realm::AffineAccessor<FT, 1, T>, true> {
public:
